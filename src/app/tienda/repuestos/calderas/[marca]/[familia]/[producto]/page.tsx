...
function getProductData(marca: string, familia: string, productoSlug: string) {
  ...
  return {
    ...
    fullName: `${familiaInfo.name} para Caldera ${marcaInfo.name} Modelo ${modelNum}`,
    ...
    description: `${familiaInfo.name} original/compatible para calderas ${marcaInfo.name}. Este componente es esencial para el correcto funcionamiento de tu caldera. Referencia del fabricante: ${refPrefix}-${refNum}.`,
    specifications: [
...
export default function ProductoPage() {
  ...
  const productJsonLd = {
    ...
    "category": `Repuestos > Calderas > ${producto.brand} > ${producto.family}`,
    ...
    "isRelatedTo": relacionados.map(r => ({
      "@type": "Product",
      "name": r.name,
      "url": `/tienda/repuestos/calderas/${marca}/${familia}/${r.slug}`
    }))
  };
  ...
  const breadcrumbJsonLd = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 4, "name": producto.brand, "item": `https://uniclima.es/tienda/repuestos/calderas/${marca}` },
      { "@type": "ListItem", "position": 5, "name": producto.family, "item": `https://uniclima.es/tienda/repuestos/calderas/${marca}/${familia}` },
      { "@type": "ListItem", "position": 6, "name": producto.reference, "item": `https://uniclima.es/tienda/repuestos/calderas/${marca}/${familia}/${productoSlug}` }
    ]
  };
        ...
        <div className="bg-white border-b border-gray-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
            <nav className="flex items-center gap-2 text-gray-500 text-sm flex-wrap">
              ...
              <Link href="/tienda/repuestos" className="hover:text-orange-600 transition-colors">Repuestos</Link>
              <ChevronRight className="w-4 h-4" />
              <Link href="/tienda/repuestos/calderas" className="hover:text-orange-600 transition-colors">Calderas</Link>
              <ChevronRight className="w-4 h-4" />
              <Link href={`/tienda/repuestos/calderas/${marca}`} className="hover:text-orange-600 transition-colors">{producto.brand}</Link>
              <ChevronRight className="w-4 h-4" />
              <Link href={`/tienda/repuestos/calderas/${marca}/${familia}`} className="hover:text-orange-600 transition-colors">{producto.family}</Link>
              <ChevronRight className="w-4 h-4" />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {relacionados.map((rel) => (
                <Link
                  key={rel.id}
                  href={`/tienda/repuestos/calderas/${marca}/${familia}/${rel.slug}`}
                  className="bg-white rounded-xl p-4 border border-gray-100 hover:border-orange-300 hover:shadow-lg transition-all duration-300"
                >
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(familiasData)
                ...
                .map(([slug, info]) => (
                  <Link 
                    key={slug}
                    href={`/tienda/repuestos/calderas/${marca}/${slug}`}
                    className="px-3 py-1.5 bg-gray-100 hover:bg-orange-100 text-gray-700 hover:text-orange-700 rounded-full text-sm transition-colors"
                  >
                    {info.name}
                  </Link>
                ))
